...
//    HalUARTRead (port, &ch, 1);
    HalUARTReadAndFormate (port, &ch, 1);
    //HalUARTWrite(0,&ch,1);
...
//        bytesInRxBuffer = Hal_UART_RxBufLen(port);
        bytesInRxBuffer = (Hal_UART_RxBufLen(port)/2);
...
//          HalUARTRead (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], bytesInRxBuffer);
          HalUARTReadAndFormate (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], bytesInRxBuffer);
          tempDataLen += bytesInRxBuffer;
        }
        else
        {
//          HalUARTRead (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], LEN_Token - tempDataLen);
          HalUARTReadAndFormate (port, &pMsg->msg[MT_RPC_FRAME_HDR_SZ + tempDataLen], LEN_Token - tempDataLen);
