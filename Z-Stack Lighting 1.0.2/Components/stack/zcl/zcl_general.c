...
static ZStatus_t zclGeneral_HdlInManuSpecificCommands( zclIncoming_t *pInMsg );//add my own function
...
static ZStatus_t zclGeneral_ProcessInManuOnOff( zclIncoming_t *pInMsg, zclGeneral_AppCallbacks_t *pCBs );
#endif // ZCL_ONOFF

#ifdef ZCL_LEVEL_CTRL
static ZStatus_t zclGeneral_ProcessInLevelControl( zclIncoming_t *pInMsg, zclGeneral_AppCallbacks_t *pCBs );
static ZStatus_t zclGeneral_ProcessInManuLevelControl( zclIncoming_t *pInMsg, zclGeneral_AppCallbacks_t *pCBs );
...
static ZStatus_t zclGeneral_HdlIncoming( zclIncoming_t *pInMsg )
{
  ...
  if ( zcl_ClusterCmd( pInMsg->hdr.fc.type ) )
  {
    ...
    else
    {
      ...
      stat = zclGeneral_HdlInManuSpecificCommands( pInMsg );  //add my own function
    }
  }
  else
  {
...
/*********************************************************************
 * @fn      zclGeneral_HdlInManuSpecificCommands
 *
 * @brief   Callback to process incoming Manufacturer Commands specific
 *          to this cluster library
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclGeneral_HdlInManuSpecificCommands( zclIncoming_t *pInMsg )//my own function
{
  ZStatus_t stat;
  zclGeneral_AppCallbacks_t *pCBs;
...
#ifdef ZCL_ON_OFF
    case ZCL_CLUSTER_ID_GEN_ON_OFF:
      stat = zclGeneral_ProcessInManuOnOff( pInMsg, pCBs );
      break;
#endif // ZCL_ON_OFF

#ifdef ZCL_LEVEL_CTRL
    case ZCL_CLUSTER_ID_GEN_LEVEL_CONTROL:
      stat = zclGeneral_ProcessInManuLevelControl( pInMsg, pCBs );
      break;
#endif // ZCL_LEVEL_CTRL
...
#ifdef ZCL_ON_OFF
/*********************************************************************
 * @fn      zclGeneral_ProcessInManuCmdOnOff
 *
 * @brief   Process in the received On/Off Command.
 *
 * @param   pInMsg - pointer to the incoming message
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclGeneral_ProcessInManuOnOff( zclIncoming_t *pInMsg,
                                            zclGeneral_AppCallbacks_t *pCBs )//add my own function
{
  ZStatus_t stat = ZSuccess;
  ...
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    ...
    {
      case COMMAND_OFF:
       dali_cmd = pInMsg->pData[0];
       dali_cmd = (dali_cmd << 1) | 0x01;
       dali_cmd = dali_cmd << 8;
...
#ifdef ZCL_LEVEL_CTRL
/*********************************************************************
 * @fn      zclGeneral_ProcessInManuLevelControl
 *
 * @brief   Process in the received Level Control Command.
 *
 * @param   pInMsg - pointer to the incoming message
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclGeneral_ProcessInManuLevelControl( zclIncoming_t *pInMsg,
                                                   zclGeneral_AppCallbacks_t *pCBs )//add my own function
{
  ZStatus_t stat = ZSuccess;
  ...
  if ( zcl_ServerCmd( pInMsg->hdr.fc.direction ) )
  {
    ...
    {
      case COMMAND_LEVEL_MOVE_TO_LEVEL:
       dali_cmd = pInMsg->pData[3];
       dali_cmd = (dali_cmd << 1) | 0x00;
       dali_cmd = (dali_cmd << 8) | (pInMsg->pData[0]);
       DALI_Send(dali_cmd,&dali_answer,&dali_answer_state);
       stat = ZSuccess;
        break;
