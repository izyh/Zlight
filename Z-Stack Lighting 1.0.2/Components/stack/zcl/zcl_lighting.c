...
//uint16 dali_cmd;
//uint8 dalishortaddr ,dali_answer ,dali_answer_state;//define my own variables
...
static ZStatus_t zclLighting_HdlInManuSpecCommands( zclIncoming_t *pInMsg );//define my own function
...
static ZStatus_t zclLighting_ProcessInManuBallastConfigCmds( zclIncoming_t *pInMsg,zclLighting_AppCallbacks_t *pCBs );  //define my own function
...
{   
  ...
  if ( zcl_ClusterCmd( pInMsg->hdr.fc.type ) )
  {
    ...
    else
    {
      ...
      stat = zclLighting_HdlInManuSpecCommands( pInMsg );//define my own function
    }
  }
  else
  {
...
/*********************************************************************
 * @fn      zclLighting_HdlInManuSpecCommands
 *
...
static ZStatus_t zclLighting_HdlInManuSpecCommands( zclIncoming_t *pInMsg )  //define my own function
{
  ...
  switch ( pInMsg->msg->clusterId )
  {
    ...
    case ZCL_CLUSTER_ID_LIGHTING_BALLAST_CONFIG:
      stat = zclLighting_ProcessInManuBallastConfigCmds( pInMsg, pCBs );
      break;
...
/*********************************************************************
 * @fn      zclLighting_ProcessInManuBallastConfigCmds
 *
 * @brief   Callback from ZCL to process incoming Commands specific
 *          to this cluster library on a command ID basis
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclLighting_ProcessInManuBallastConfigCmds( zclIncoming_t *pInMsg,
                                             zclLighting_AppCallbacks_t *pCBs )  //define my own function
{
  ZStatus_t stat;
  ...
  {
    uint8 Tbuf[1];
    case COMMAND_BALLAST_DALI_ADDRESSING:
      for(uint8 i=0; (dalishortaddr-i) > 0; i++)
      {
       Tbuf[0] = i;
       zcl_SendCommand(pInMsg->msg->endPoint,&(pInMsg->msg->srcAddr),ZCL_CLUSTER_ID_LIGHT_LINK,
                       0x43,1,1,
                       0,pInMsg->hdr.manuCode,pInMsg->hdr.transSeqNum,1,Tbuf);//send a notice to controller after allocate DALI address.
      }
//       zcl_SendCommand(0x0b,&(pInMsg->msg->srcAddr),ZCL_CLUSTER_ID_LIGHT_LINK,
//                       0x43,1,1,
//                       0,pInMsg->hdr.manuCode,pInMsg->hdr.transSeqNum,1,Tbuf);//send a notice to controller after allocate DALI address.
      P0DIR = 0x02;
      flag=!flag;
