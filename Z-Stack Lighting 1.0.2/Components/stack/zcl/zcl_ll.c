...
static ZStatus_t zclLL_HdlInManuSpecificCommands( zclIncoming_t *pInMsg );//add my own function
static ZStatus_t zclLL_ProcessInLLCmds( zclIncoming_t *pInMsg, zclLL_AppCallbacks_t *pCBs );
static ZStatus_t zclLL_ProcessInManuLLCmds( zclIncoming_t *pInMsg, zclLL_AppCallbacks_t *pCBs );//add my own function
...
static ZStatus_t zclLL_HdlIncoming(  zclIncoming_t *pInMsg )
{
  ...
  if ( zcl_ClusterCmd( pInMsg->hdr.fc.type ) )
  {
    ...
    else
    {
      ...
      status = zclLL_HdlInManuSpecificCommands( pInMsg );//add my own function
    }
  }
  else
  {
...
/*********************************************************************
 * @fn      zclLL_HdlInManuSpecificCommands
 *
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclLL_HdlInManuSpecificCommands( zclIncoming_t *pInMsg )
{
  ZStatus_t status;
  zclLL_AppCallbacks_t *pCBs;
  ...
  {
    case ZCL_CLUSTER_ID_LIGHT_LINK:
      status = zclLL_ProcessInManuLLCmds( pInMsg, pCBs );
      break;
...
/*********************************************************************
 * @fn      zclLL_ProcessInManuLLCmds
 *
 * @brief   Callback from ZCL to process incoming Manufacturer Commands specific
 *          to this cluster library on a command ID basis
 *
 * @param   pInMsg - pointer to the incoming message
 * @param   pCBs - pointer to callbacks
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclLL_ProcessInManuLLCmds( zclIncoming_t *pInMsg, zclLL_AppCallbacks_t *pCBs )
{
  ZStatus_t status = SUCCESS;
#ifdef MT_APP_FUNC
  uint8 buf[8];
...
#ifdef MT_APP_FUNC        
      case COMMAND_ZLL_GET_DALI_ADDR_LIST_RSP:
        buf[0]=0xfe;
        buf[1]=0x03;
        buf[2]=0x49;
        buf[3]=0x82;
        buf[4]=pInMsg->pData[0];
        buf[5]=pInMsg->msg->srcAddr.addr.shortAddr;
        buf[6]=pInMsg->msg->srcAddr.addr.shortAddr >> 8;
        buf[7]=0;
        HalUARTWrite(MT_UART_DEFAULT_PORT,buf,8);
        status = ZSuccess;
        break;
#endif
